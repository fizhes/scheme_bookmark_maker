...
let poi = new Image();
poi.src = 'https://i.postimg.cc/j5FcrLZj/chillpoi.png';
poi.className = 'DqMRee SsAred';
...
function cool_() {
  ...
  const poi_ = document.getElementById('poi').checked;
  const score_bar     = hex_to_rgb(document.getElementById('score_bar').value     || '#4A752C');
  const border        = hex_to_rgb(document.getElementById('border').value        || '#578A34');
  const walls         = hex_to_rgb(document.getElementById('walls').value         || '#578A34');
  const shadows       = hex_to_rgb(document.getElementById('shadows').value       || '#94BD46');
  ...
  poi_ && (menu_ctx.drawImage(
    poi,
    menu_preview.width * 324 / 400, menu_preview.height * 58 / 542,
    menu_preview.width * 47 / 400, menu_preview.width * 47 / 400
  ));
  //custom draw image
}
...
document.getElementById('poi').oninput =
...
document.getElementById('save').onclick = function() {
  ...
  const lime = document.getElementById('lime').checked;
  const pepper = document.getElementById('pepper').checked;
  const poi = document.getElementById('poi').checked;
  ...
  const p = document.getElementById('poison').value;

  const ccanv = document.createElement('canvas');
  ccanv.width = ccanv.height = 128;
  const cctx = ccanv.getContext('2d');
  cctx.drawImage(cus, 0, 0, 128, 128);
  const _url = ccanv.toDataURL();
